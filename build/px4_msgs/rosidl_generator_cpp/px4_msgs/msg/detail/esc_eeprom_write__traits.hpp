  ...
  std::ostream & out)
{
  ...
  // member: write_mask
  {
    if (msg.write_mask.size() == 0) {
      out << "write_mask: []";
    } else {
      out << "write_mask: [";
      size_t pending_items = msg.write_mask.size();
      for (auto item : msg.write_mask) {
        rosidl_generator_traits::value_to_yaml(item, out);
        if (--pending_items > 0) {
          out << ", ";
        }
...
inline std::string to_yaml(const EscEepromWrite & msg, bool use_flow_style = false)
{
...
inline void to_yaml(
  const px4_msgs::msg::EscEepromWrite & msg,
  std::ostream & out, size_t indentation = 0)
{
...
inline std::string to_yaml(const px4_msgs::msg::EscEepromWrite & msg)
{
